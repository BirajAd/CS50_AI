...
class NimAI():
    ...
    def update(self, old_state, action, new_state, reward):
        ...
        # print((old_state, action, old, reward, best_future))
    ...
    def update_q_value(self, state, action, old_q, reward, future_rewards):
        ...
        ans = old_q + self.alpha * (new_value_estimate - old_q)
        # print(old_q,"+",self.alpha,"* (",reward,"+",future_rewards,"-",old_q,")")
        self.q[tuple(state), action] = ans
        ...
        if(len(aa) > 0):
            for i in aa:
                if (tuple(state), i) in self.q:
                    if(self.q[tuple(state), i] > max_q):
                        max_q = self.q[tuple(state), i]
                    else:
                        max_q = 0
                else:
                    self.q[tuple(state), i] = 0
        else:
            return 0
        return max_q
